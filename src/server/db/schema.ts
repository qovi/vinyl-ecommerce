...
export const vinyls = createTable(
	...
	(d) => ({
		...
		artist: d.text({ length: 256 }).notNull(),
		image: d.text({ length: 256 }).notNull(),
		price: d.integer({ mode: "number" }).notNull(),
		release: d.text({ length: 256 }).notNull(),
		media_condition: d.text({ length: 256 }).notNull(),
		sleeve_condition: d.text({ length: 256 }).notNull(),
		genre: d.text({ length: 256 }).notNull(),
